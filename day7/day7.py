...
complete = []
ready = sorted([x for x in children.keys() if x not in [y for sublist in children.values() for y in sublist]])
...
while i < len(queue):
    step = queue[i]
    i += 1
    if step in ready or not [p for p in children.keys() if p not in complete and step in children[p]]:
